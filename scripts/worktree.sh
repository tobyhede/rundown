...
#        pnpm run worktree -- <name> [base-ref]
#
# Creates .worktrees/<name> on a new branch <name>, branched from [base-ref]
# (default: main), then runs `pnpm install` inside it so the workspace is
...
# that step makes `pnpm run build`/tests fail with confusing cross-package type
