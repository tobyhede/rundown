...
describe('FOR loop transitions integration', () => {
  ...
  describe('FOR loop aggregation', () => {
    it('PASS ALL — all iterations pass → CONTINUE to next step', async () => {
      ...
      let result = runCli('run --prompted agg-all-pass.runbook.md --text', workspace);
    ...
    it('PASS ALL — one iteration fails via BREAK → BREAK does not accumulate', async () => {
      ...
      // Start runbook in prompted mode
      expect(runCli('run --prompted agg-one-fail.runbook.md --text', workspace).exitCode).toBe(0);

      // Iteration 1: pass → DEFER feeds 'pass' → iteration DEFER → loop back to iteration 2
      expect(runCli('pass --text', workspace).exitCode).toBe(0);
    ...
    it('PASS ANY — one pass suffices', async () => {
      ...
      // Start runbook
      expect(runCli('run --prompted agg-any-pass.runbook.md --text', workspace).exitCode).toBe(0);

      // Iteration 1: fail → DEFER feeds 'fail' → iteration DEFER → loop back
      expect(runCli('fail --text', workspace).exitCode).toBe(0);
    ...
    it('FAIL ANY: BREAK at iteration level — BREAK does not accumulate', async () => {
      ...
      // Start runbook
      expect(runCli('run --prompted agg-break.runbook.md --text', workspace).exitCode).toBe(0);
  ...
  describe('FOR iteration-level RETRY', () => {
    it('RETRY 2 BREAK — exhausts retries then breaks', async () => {
      ...
      // Start runbook
      expect(runCli('run --prompted retry-break.runbook.md --text', workspace).exitCode).toBe(0);
    ...
    it('RETRY 1 DEFER — retries then defers to next iteration', async () => {
      ...
      // Start runbook
      expect(runCli('run --prompted retry-defer.runbook.md --text', workspace).exitCode).toBe(0);
    ...
    it('RETRY succeeds on second attempt', async () => {
      ...
      // Start runbook
      expect(runCli('run --prompted retry-success.runbook.md --text', workspace).exitCode).toBe(0);
...
`;
      ...
      // Start runbook in prompted mode
      expect(runCli(`run --prompted ${filename} --text`, workspace).exitCode).toBe(0);
