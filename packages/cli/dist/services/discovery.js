 ...
 */
export async function scanDirectory(dirPath, source) {
    const runbooks = [];
    try {
        ...
        for (const entry of entries) {
            if (!entry.isFile())
                continue;
            if (!entry.name.endsWith('.runbook.md'))
                continue;
            try {
 ...
 */
export async function discoverRunbooks(cwd) {
 ...
 */
export async function findRunbookByName(cwd, name) {
    const searchPaths = getSearchPaths(cwd);
    for (const { path: dirPath, source } of searchPaths) {
        const runbooks = await scanDirectory(dirPath, source);
        for (const runbook of runbooks) {
