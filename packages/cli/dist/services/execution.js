...
import { printActionBlock, printStepBlock, printSeparator, printCommandExec, printWorkflowComplete, printWorkflowStoppedAtStep, executeCommand, createStepNumber, } from '@rundown/core';
/**
 * Check if workflow snapshot indicates completion
 */
export function isWorkflowComplete(snapshot) {
    return snapshot.status === 'done' && snapshot.value === 'COMPLETE';
}
/**
 * Check if workflow snapshot indicates stopped state
 */
export function isWorkflowStopped(snapshot) {
 ...
 */
export async function runExecutionLoop(manager, workflowId, steps, cwd, prompted, agentId) {
    ...
    while (true) {
        ...
        const action = deriveAction(prevStep, updatedState.step, prevSubstep, updatedState.substep, prevRetryCount, updatedState.retryCount, retryMax, isComplete, isStopped);
 ...
 */
export function isValidResult(r) {
    return r === 'pass' || r === 'fail';
}
/**
 * Get retry max for a step
 */
export function getStepRetryMax(step) {
 ...
 */
export function deriveAction(prevStep, newStep, prevSubstep, newSubstep, prevRetryCount, newRetryCount, retryMax, isComplete, isStopped) {
    if (isComplete)
        return 'COMPLETE';
    if (isStopped)
        return 'STOP';
