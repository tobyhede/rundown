    ...
    program
        ...
        await withErrorHandling(async () => {
            ...
            const toDelete = states.filter((state) => {
                ...
                if (pruneCompleted && isCompleted)
                    return true;
                if (pruneActive && isActive)
                    return true;
                if (pruneInactive && isInactive)
                    return true;
                return false;
            });
...
function getStatus(state, activeState, stashedId) {
    if (activeState?.id === state.id)
        return 'active';
    if (state.id === stashedId)
        return 'stashed';
