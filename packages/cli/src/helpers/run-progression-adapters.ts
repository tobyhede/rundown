...
export interface RunProgressionDriveContext {
  ...
  readonly output: OutputEmitter;
  /**
   * Required: the invoking command already bridged an emitter to `output` for
   * its own observation, and the activation must share it so one `seq` counter
   * covers the whole command. A fallback emitter here would silently start a
   * second stream.
   */
...
export async function driveRunProgression(
  ...
  if (outcome.kind === 'failed') {
    try {
      ...
      // FLUSHED here, not left to the caller's end-of-command flush (#853):
      // `output.error` only accumulates into the JSON renderer while a
      // command's deferred action object is written straight through, so an
      // envelope left in the accumulator lands AFTER the object every caller
      // defers to keep last (docs/spec/cli-output.md). Flushing an empty
      // accumulator is a no-op, so this costs nothing on the other arms.
      ctx.output.flush();
    } catch {
