...
The subagent must pass the claim id to every child command (`rd status/pass/fail --claim-id <claim_id>`); bare `rd pass`/`rd fail` targets the parent, not the child.
