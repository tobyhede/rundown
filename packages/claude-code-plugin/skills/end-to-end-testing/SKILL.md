...
- Treat `rundown run`, `rundown pass`, `rundown fail`, `rundown claim`, and `rundown collect` output as the next context. Parent-side pass/fail/collect carry `--run-capability <run_capability>` (capture the run id from `rundown run` output, echoed as `runbookId` on every event); child-side commands carry `--claim-capability <claim_capability>`.
...
Drive the claimed child like any runbook: advance each step with `rundown pass --claim-capability <claim_capability>` / `rundown fail --claim-capability <claim_capability>`. Prompted child steps need this claim-capability transition to advance. On a delegation-exposed run, a bare `rundown pass`/`rundown fail` is refused with `ACTOR_CONTEXT_REQUIRED` regardless of claim state (exposure is sticky). A `--run-capability`-targeted parent advance is additionally refused with `OPEN_DELEGATED_CHILDREN` while a claimed child is open.

When the child's final step completes it auto-resolves its parent substep and the parent auto-aggregates and advances. `rundown pass/fail --claim-capability` is idempotent on a resolved child, so it also confirms or overrides a child you stop early.
