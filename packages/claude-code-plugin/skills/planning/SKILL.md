...
Capture the runCapability from that command: `rundown run` includes it in JSON output. This pipeline delegates (step 1), so every orchestrator command you issue — `collect`, `pass`, `goto` — must carry `--run-capability <run_capability>`.
