...
const CLOSURE_VIOLATION =
  'Delegated Rundown work was active when the subagent stopped. Run `rundown status` to discover the active delegation, then close it explicitly in your own lane: if a claim capability was issued, use `rundown pass --claim-capability <capability>` or `rundown fail --claim-capability <capability>`; if the token was never claimed, either claim and close it with the returned claim capability, or report the token back so the orchestrator can `rundown delegate --retry <token> --run-capability <capability>` from its own context. Cancel with `rundown abort <token>`.';
 ...
 *   claimed work is recovered via `rundown pass --claim-capability` or
 *   `rundown fail --claim-capability`, and an unclaimed token is either claimed
 *   and closed with the returned claim capability, or reported back so the
 *   orchestrator retries it from its own context with
 *   `rundown delegate --retry <token> --run-capability <capability>` (or
 *   `rundown abort <token>`).
