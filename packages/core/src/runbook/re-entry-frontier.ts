 ...
 *
 * Deliberately narrower than the full {@link RunbookActorService}: the fenced
 * runner DERIVES its consume rather than committing one, so this surface exposes
 * only `prepareActorMutation` and a caller cannot reach a committing write
 * through it.
 */
 ...
 *
 * Classifies every arm the seam can reach EXCEPT `consume_failed`: a derivation
 * cannot half-commit, so the only way the consume does not land is that the
 * enclosing transaction refused, which the caller reports as its own
 ...
 *
 * The only route to a re-entry consume, and it never commits one itself: it
 * returns the prepared state plus the entry metadata to observe, leaving both
 * the commit and the disclosure to the caller's transaction. The seam that used
 * to commit the consume and return rendered observations was deleted with the
 * CLI-owned progression path (#858).
 *
 * The ordering guarantee is preserved and in fact strengthened: the deleted
 * seam guaranteed "no bearers disclosed unless the consume committed" by
