...
async function driveProgression(
  ...
  let progressionFeedback: RunProgressionMachineFeedback = {
    kind: 'activation',
  };
  ...
  while (true) {
    ...
    if (progressionIntent.kind === 'reselect') {
      currentState = progressionIntent.state;
      continue;
    }
    if (progressionIntent.kind === 'refused') {
      if (progressionIntent.reason === 'run_missing') return runMissingRefusal(runId, sink);
      if (progressionIntent.reason === 'actor_context_required') {
        sink.emit({
          type: 'ERROR_OCCURRED',
          payload: {
      ...
      if (progressionIntent.reason === 'projection_refused') {
        const message = `${FRONTIER_PROJECTION_REFUSED_MESSAGE}: ${progressionIntent.message}`;
        sink.emit({
          type: 'ERROR_OCCURRED',
          payload: {
            message,
      ...
      if (progressionIntent.reason === 'consume_failed') {
        sink.emit({
          type: 'ERROR_OCCURRED',
          payload: {
      ...
      if (progressionIntent.reason === 'frontier_disclosure_failed') {
        if (progressionIntent.cause instanceof InvalidRunbookStateError)
          throw progressionIntent.cause;
        const error = Errors.frontierDisclosureFailed(runId, progressionIntent.message);
        sink.emit({
          type: 'ERROR_OCCURRED',
      ...
      if (progressionIntent.reason === 'claim_superseded') {
      ...
      const isAggregate = progressionIntent.reason === 'aggregate_recovery_required';
    ...
    if (progressionIntent.kind !== 'entered') {
      throw new Error(`Unhandled Run Progression intent: ${progressionIntent.kind}`);
    }
    currentState = progressionIntent.state;
    const entered: ExecutionUnitEntry = progressionIntent.entered;
    ...
    if (progressionIntent.frontier === 'none' && entered.kind === 'inline-launch') {
    ...
    const currentStep = findStepOrThrow(steps, previousState.step, previousState.id);
