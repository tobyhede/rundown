...
export function createDelegation(
  ...
  //    three, and no BEHAVIOURAL test can fail — while this guard holds, the
  //    state each would have to handle cannot be built. That is why they are
  //    listed here rather than left to be rediscovered. The first two both
  //    classify the bearer with `isDelegatedChildClaim` (`claim-id.ts`), so
  //    grepping that predicate finds every exemption this invariant underwrites.
  //
  //    The list is enforced rather than merely written: each SITE carries an
  //    `RD-819-DEPENDENT` marker naming it, and
  //    `__tests__/runbook/rd819-dependents.source-text.test.ts` holds the marked
  //    sites and the marked entries equal in both directions. Detaching this
  //    block from the guard, dropping an entry, or adding a fourth marked
  //    dependent without listing it all fail there (#703). The count word
  //    opening this paragraph counts MARKED SITES and is read by that test.
  //    A bullet that elaborates a site already marked above carries no marker of
  //    its own — it is a second reason at one site, not a second site, and it
  //    must not move the count:
  //      - RD-819-DEPENDENT: skipOpenClaims — `resolveTransitionTarget`'s
  //        exemption (`command-target-resolver.ts`) skips the parent open-claim
  //        read for a delegated-child bearer, sound only because such a run can
  //        have no delegated children of its own.
  //      - RD-819-DEPENDENT: guardOpenChildren —
  //        (`lifecycle-command-service.ts`) exempts that same bearer from the
  //        in-transaction open-children guard, for the same reason. Lifting this
  //        prohibition without revisiting it reintroduces #700 one level down.
  ...
  //        one. Unmarked: it is the `guardOpenChildren` site above.
  //      - RD-819-DEPENDENT: transitionDelegationRuntime —
  //        (`lifecycle-command-service.ts`) keeps a structural rather than
  //        behavioural grant check, which cannot be exploited only because the
  //        sole bearer holding `mutate-run` without `delegate-from-run` is a
  //        delegated child's.
