 ...
 * DELEGATION linkage — reachable, because a delegation-linked child reaches the
 * report seam without a claim gate of its own. Wording that asserts "inline"
 * would be flatly false for such a graph and would send the operator hunting
 * for a composition that does not exist. "Parent linkage" is
