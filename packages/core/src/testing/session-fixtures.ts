...
/**
 * Patch an already-persisted claim's columns directly.
 *
 * `saveSession` cannot do this. It reconciles claims rather than rewriting them:
 * an already-persisted claim is left untouched, so a wholesale save never churns
 * `claim_generation` nor clobbers the #519 `last_seen_at` liveness mark. That is
 * deliberate, and it means the pre-SQLite fixture pattern — `loadSession`, edit
 * `session.claims[key]`, `saveSession` — now silently no-ops for an existing
 * claim.
 *
 * Tests that must stage a claim the public API will not mint (an idle
 * `lastSeenAt`, a claim missing a grant, a run-control claim with its delegation
 * linkage stripped) therefore write the columns here, in the same spirit as
 * `takeOwnership` in `runbook-store.test.ts`. Production code must keep using
 * the claim lifecycle seams (`insertClaim`, `tombstoneClaim`, `recordClaimSeen`).
 *
 * `delegation` is all-or-nothing: `parent_run_id` and `parent_linkage_version`
 * are written alongside `delegation_json` so the row stays internally consistent,
 * matching how `insertClaim` mints it.
 *
 * @param cwd - Project root whose store holds the claim.
 * @param claimKey - Lookup key of the claim to patch.
 * @param patch - Columns to change; omitted fields are left as stored.
 * @param patch.lastSeenAt - New #519 activity timestamp.
 * @param patch.grants - Replacement grant list.
 * @param patch.delegation - Replacement delegation linkage, or `null` to strip it.
 * @returns Resolves once the row is committed.
 * @throws {Error} When no claim carries `claimKey`, so a typo cannot silently no-op.
 */
export async function patchPersistedClaim(
  cwd: string,
  claimKey: string,
  patch: {
    readonly lastSeenAt?: string;
    readonly grants?: readonly unknown[];
    readonly delegation?: Record<string, unknown> | null;
  },
): Promise<void> {
  const store = await getRunbookStore(cwd);
  await store.transaction((txn) => {
    const present = txn.tx
      .prepare('SELECT 1 AS present FROM claims WHERE key = :key')
      .get<{ readonly present: number }>({ key: claimKey });
    if (present === undefined) {
      throw new Error(`patchPersistedClaim: no persisted claim ${claimKey}`);
    }
    if (patch.lastSeenAt !== undefined) {
      txn.tx
        .prepare('UPDATE claims SET last_seen_at = :lastSeenAt WHERE key = :key')
        .run({ key: claimKey, lastSeenAt: patch.lastSeenAt });
    }
    if (patch.grants !== undefined) {
      txn.tx
        .prepare('UPDATE claims SET grants_json = :grantsJson WHERE key = :key')
        .run({ key: claimKey, grantsJson: JSON.stringify(patch.grants) });
    }
    if (patch.delegation !== undefined) {
      const delegation = patch.delegation;
      txn.tx
        .prepare(
          `UPDATE claims
              SET delegation_json = :delegationJson,
                  parent_run_id = :parentRunId,
                  parent_linkage_version = :parentLinkageVersion
            WHERE key = :key`,
        )
        .run({
          key: claimKey,
          delegationJson: delegation ? JSON.stringify(delegation) : null,
          parentRunId: typeof delegation?.parentRunId === 'string' ? delegation.parentRunId : null,
          parentLinkageVersion: delegation ? 0 : null,
        });
    }
  });
}
