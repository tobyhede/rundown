 ...
 * 2. The shell was killed by the harness timeout partway through. The trace is
 *    then truncated, and an absence of unauthorized heads in it is not evidence
 *    that none ran.
 *
...
describe('runInSandbox: a failed spawn is not an observation', () => {
  ...
  it('still reports a genuine empty trace from a shell that ran', () => {
    ...
    const { heads, timedOut } = runInSandbox(sandbox, 'no-such-command');
