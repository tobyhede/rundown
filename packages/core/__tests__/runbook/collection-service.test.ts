...
describe('RunbookCollectionService', () => {
  ...
  it('returns a progression directive and leaves retry frontier ownership to XState', async () => {
    ...
    // A projected-frontier entry the collect must NOT perform: since #855 the
    // frontier turn belongs to Run Progression, so this spy exists only to
    // prove it is never reached (asserted below).
  ...
  it('commits the terminal state, its release, and its parent report as one unit', async () => {
    ...
    // lifecycle, so a rejecting session release could leave a completed run on
