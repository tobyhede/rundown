...
describe('openRunbookDriver sql.js startup failure', () => {
  it.each([new Error('wasm failed to load'), 'bare string init failure'])(
    'wraps %p as a typed hard startup refusal without a native fallback',
    async (failure) => {
...
describe.each(['native', 'sqljs'] as const)(
  'openRunbookDriver %s schema initialization failure',
  (runtime) => {
    it.each([false, true])(
      'disposes the driver without masking the schema error',
      async (failClose) => {
        ...
        await expect(openRunbookDriver('does-not-matter.db', { runtime })).rejects.toBe(
          schemaError,
        );
