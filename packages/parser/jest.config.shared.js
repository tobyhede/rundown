...
export function makeConfig({ sandboxed }) {
  ...
  // Repo-asset meta-tests read a file that lives OUTSIDE this package — the
  // repo-root `runbooks/` tree, a `scripts/**` build script — via a relative
  // traversal off `import.meta.url`. The sandbox copy adds two path segments, so
  // that traversal lands on `packages/parser/<path>` and the asset is gone. The
  // failure is a HARD dry-run abort that kills the campaign before a single
