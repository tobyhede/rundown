...
- **Transactional refusals under `rundown collect`.** The whole collection
  ...
  delegation re-entry frontier consumption, the terminal session release, and a
  delegating grandparent's outcome row land together or not at all. Collect's
  own refusal envelope therefore reports both the policy and collection outcomes
  (`ACTOR_CONTEXT_REQUIRED`, `CLAIM_GRANT_REQUIRED`, `DELEGATION_SUPERSEDED` /
  `CLAIMED_RUNBOOK_UNAVAILABLE`, `SUBSTEPS_NOT_RESOLVED`, `NOT_DELEGATE_STEP` /
  `STEP_NOT_FOUND` / `COLLECT_OPERATION_FAILED`, RD-821) and the transactional
  ones (`STALE_CLAIM`, `CONCURRENT_MODIFICATION`, `EXECUTION_IN_PROGRESS`,
  ...
  rather than committing under stale authority. `RD-829` is **not** reachable
  from collect: a collection derives its frontier consume rather than committing
  one separately, so the only way it does not land is that the enclosing
  transaction refused — reported under the transactional code, with the frontier
  likewise untouched and the operation retryable. `RUN_TARGET_UNAVAILABLE` has
  two origins here that share a remedy: `--run` naming a run that is not on the
  active stack (nothing captured), and a captured run disappearing
