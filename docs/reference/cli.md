...
| `COMPLETION_TARGET_MISMATCH`                       | A persisted completion is not for the run's active cursor, so the drain refused to apply it. Permanent — re-running cannot make a stale completion match a cursor that has moved. Reported identically by the execution loop (the run stops) and by the inline parent-advance seam; `rundown collect` reports the same underlying fact as `COLLECT_OPERATION_FAILED`, which names the collection rather than the cursor                                                                                                                                                        | Do not retry. The message names the current cursor and the completion's target; finish, stop, or prune the run and restart it from source                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
