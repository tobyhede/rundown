...
**Fallback resolution:** `--dir` is optional when `rdpath` can infer a run work path. The base directory resolves from explicit `--dir`, then `$RD_WORK_PATH`, then active runbook `WorkPath`. `--ctx` resolves from explicit `--ctx`, then `$RD_CONTEXT_ID`; active runbook `ContextId` is used only when active state was already needed to resolve the base directory. The environment variables are injected automatically by `rundown` for each shell block; the active-state fallback lets the same bare `rdpath --file ...` form work from another terminal while a runbook is active. Explicit `--dir` or `$RD_WORK_PATH` calls do not read active state, so stale or corrupt runbook state is non-fatal for otherwise complete invocations. If no base directory can be resolved, `rdpath` writes `error: --dir is required (or set $RD_WORK_PATH)` to stderr and exits with code `1`. `--ctx` remains optional — when no flag, env var, or active `ContextId` is available, the path is assembled without a context segment.
