...
| Gate | Result |
| --- | --- |
| Conflicts | 6 hunks / 5 files, exactly as Delta 1 predicted; `--diff-filter=U` empty, `git diff --check <recorded-base>...HEAD` exit 0 (re-measured post-commit against the recorded base SHA, not the moving `origin/main` — that ref advanced after this branch's base was established, so naming it would make the gate non-reproducible; a bare `git diff --check` on a committed branch sees only unstaged changes and so cannot fail) |
| `comm -3` vs the 13-path allowlist | five additions, zero allowlisted-but-unchanged: `cspell-dictionary.txt` (Delta 2), this addendum, `packages/core/__tests__/runbook/completion-service.test.ts` (P1 fix), `packages/core/__tests__/runbook/storage/fixtures/child-protocol.ts` (G1), `packages/core/__tests__/runbook/guarded-drain-composition.test.ts` (Delta 9) |
...
**Sixth allowlist expansion (stop-and-review).** The composition pin below is a new path outside the 13-path derived allowlist, taking `comm -3` from four additions to five. Recorded here rather than added silently, per Delta 2's precedent; the gates table and step 4 are updated to match. It earns its place because no allowlisted suite can host it — every one of them stubs the drain or mocks `sendAndSync`, which is precisely the gap it closes.
