...
The `inputs` field declares default values for template variables as a key-value map (e.g., `environment: development`). Values must be strings, numbers, or booleans. These defaults sit at low priority in the variable resolution chain — they are overridden by CLI flags, environment variables, and config files, and they take precedence over inherited delegation variables. Variable names must match the same identifier pattern as `required`. Entries must not appear in `required` (required variables have no default). Reserved runtime names (`step`, `index`, `context`, matched case-insensitively) are rejected. See [§6 Templating](#6-templating) for the full precedence order. Note: at runtime, variables are also injected from the context outputs store as gap-filling (see [§7 Context Passing](#7-context-passing-outputs)) — this is a separate runtime mechanism from the `inputs:` defaults.
