...
Claude's structured output engine enforces schemas at the grammar level. Only a subset of JSON Schema is supported server-side:
...
Custom `format` values like `"markdown"` or `"filepath"` are silently stripped by Claude's engine. The model never sees them.
...
Zod's `.nullable()` generates `oneOf` in JSON Schema, which Claude's engine doesn't support. The SDK transforms it with loss. Use `.optional()` instead — field absence is simpler and fully supported.
...
`enum` is fully supported and enforced at the grammar level. It's more reliable than describing allowed values in text.
...
`$ref` and `$defs` are not processed by Claude's engine. Inline the definition at each usage site.

Note: Zod variable reuse (e.g. using a `FileEntry` variable in multiple places) is fine — the SDK expands definitions inline automatically when converting to JSON Schema.

### 7. Add meaningful descriptions to every field

When unsupported constraints (`minLength`, `minItems`, etc.) are stripped, `description` is the model's only source of information about the constraint. Each description should explain what the field means, not just its format.
...
**Zod is the source of truth** for runtime validation behavior. The JSON Schema file is hand-maintained and must be kept in sync.
...
This means Zod constraints like `.min(1)` still work for client-side validation even though they're stripped server-side. But it's better to design schemas that don't rely on SDK transformation for correctness.
