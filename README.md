...
Rundown transforms static documentation into interactive, stateful CLI runbook
flows, bridging the gap between "how-to" and "done."
...
- **Stateful:** It remembers where you left off. Close your terminal, reboot
  your machine, and `rundown status` picks right back up.
- **Agent-Ready:** Designed to be the perfect "runtime" for AI agents (like
  Claude) to execute long-running, complex tasks reliably.
...
- **XState compilation** - Runbooks compile to state machines for reliable
  execution
...
Rundown includes a Deno-inspired security policy layer that provides explicit
allowlist-based permission control:

- **Default-deny** - Commands not in the allowlist require user confirmation
- **Granular control** - Separate allow/deny lists for commands, file access,
  and environment variables
- **Runbook overrides** - Per-runbook policy configurations for different trust
  levels
- **CI-friendly** - `--yes` flag for non-interactive execution with pre-approved
  commands
...
See [Security Documentation](docs/reference/security.md) for full configuration
options.
